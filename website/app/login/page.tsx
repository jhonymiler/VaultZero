...
export default function LoginPage() {
  ...
  // Inicializar SDK
  useEffect(() => {
    // Detectar URL de callback apropriada para desenvolvimento
    const getCallbackUrl = () => {
      const origin = window.location.origin;
      
      // Em desenvolvimento, se for localhost, usar o IP da rede para permitir acesso do mobile
      if (origin.includes('localhost') || origin.includes('127.0.0.1')) {
        // Em desenvolvimento, vamos mostrar um aviso para o usuário configurar o IP
        console.warn('🚨 IMPORTANTE: Para testar com app mobile, você precisa acessar o website pelo IP da máquina na rede local, não localhost');
        console.warn('📱 Exemplo: http://192.168.1.100:3001 ao invés de http://localhost:3001');
        console.warn('🔧 Ou configure a variável NEXT_PUBLIC_HOST_IP no .env');
      }
    ...
    const vaultSDK = new VaultZeroSDK({
      apiBaseUrl: window.location.origin, // Usar a mesma origem do website
    ...
    vaultSDK.on('authentication_success', (event) => {
      ...
      console.log('🔍 SessionId recebido:', event.data.sessionId)
      ...
      const userData = {
        ...event.data.userData,
        loginTime: new Date().toISOString() // Adicionar timestamp do login
      }
      
      const sessionData = {
        sessionId: event.data.sessionId,
        timestamp: Date.now(),
        loginTime: new Date().toISOString(), // Adicionar aqui também
        expiresAt: Date.now() + (24 * 60 * 60 * 1000)
      }
      
      console.log('💾 Salvando userData:', userData)
      console.log('💾 Salvando sessionData:', sessionData)
      
      localStorage.setItem('vaultzero_user', JSON.stringify(userData))
      localStorage.setItem('vaultzero_session', JSON.stringify(sessionData))
    ...
    vaultSDK.on('access_revoked', (event) => {
      console.log('🚫 Acesso revogado:', event.data)
      // Limpar dados locais
